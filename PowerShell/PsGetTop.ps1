...
#
# From: johansenreidar.blogspot.com/2013/10/powershell-list-processes-currently.html
#
...
Else {
  ...
  $ProcessList = gwmi Win32_PerfFormattedData_PerfProc_Process | where {$_.Name -like "*$ProcessName*"} | select IDProcess,Name,PercentProcessorTime | sort PercentProcessorTime -Descending | select -First $SelectFirst
}
$TopProcess = @()
